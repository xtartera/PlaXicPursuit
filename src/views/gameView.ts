...
export function renderGameScreen(model: GameModel): string {
  ...
  return `<main class="quiz-game"><header class="quiz-nav"><div class="quiz-brand"><span>PX</span><strong>Quiz del Pla Xic</strong></div><div class="quiz-progress"><span>Pregunta ${model.currentQuestion + 1} de ${model.questions.length}</span><div><i style="width:${model.progressPercent}%"></i></div></div><div class="score"><small>Punts</small><b>${model.score}</b></div></header><section class="quiz-question"><div class="question-meta"><span>${escapeHtml(question.categoryLabel)}</span><span>${escapeHtml(question.difficulty)}</span></div><h1>${escapeHtml(question.prompt)}</h1>${image}<div class="answers" role="group" aria-label="Opcions de resposta">${answers}</div><div class="feedback" id="feedback" hidden></div></section><footer class="quiz-footer"><a href="${suggestionFormUrl}" target="_blank" rel="noreferrer">Proposa una pregunta</a><button class="quit" type="button">Sortir del quiz</button></footer></main>`
}
