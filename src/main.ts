...
function render(resetAttempts = true): void {
  ...
  document.querySelector<HTMLButtonElement>('.board-toggle')?.addEventListener('click', showBoardModal)
}

function showBoardModal(): void {
  const backdrop = document.createElement('div')
  backdrop.className = 'modal-backdrop board-modal-backdrop'
  backdrop.innerHTML = `<div class="board-modal" role="dialog" aria-modal="true" aria-label="Progrés de la partida"><button class="modal-close" type="button" aria-label="Tancar">&times;</button><p class="eyebrow">EL TEU PROGRÉS</p>${renderBoard()}${renderCollection()}</div>`
