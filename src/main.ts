...
type Question = { id: string; category: string; categoryLabel: string; difficulty: string; prompt: string; options: string[]; answer: number; explanation: string }
...
function parseQuestions(markdown: string): Question[] {
  ...
  return blocks.flatMap((match) => {
    ...
    return [{ id: metadata.id, category: metadata.category, categoryLabel: categoryLabels[metadata.category] ?? metadata.category, difficulty: metadata.difficulty, prompt: title, options, answer, explanation }]
  })
}
...
function render(resetAttempts = true): void {
  ...
  app.innerHTML = `<main class="shell"><header class="topbar"><div class="brand-mark" aria-hidden="true">PX</div><div><p class="eyebrow">PLA XIC</p><h1>Pursuit</h1></div><div class="score"><span>Punts</span><strong>${score}</strong></div></header>${renderBoard()}<section class="game-panel" aria-live="polite"><div class="progress-row"><span>Pregunta ${currentQuestion + 1} de ${questions.length}</span><span>${progress}%</span></div><div class="progress-track"><span style="width: ${progress}%"></span></div><div class="question-meta"><span class="category-dot"></span><span>${question.categoryLabel}</span><span class="difficulty">${question.difficulty}</span></div><h2>${question.prompt}</h2><div class="answers" role="group" aria-label="Opcions de resposta">${question.options.map((option, index) => `<button class="answer ${wrongAnswers.has(index) ? 'incorrect' : ''}" data-index="${index}" type="button" ${wrongAnswers.has(index) ? 'disabled' : ''}><span class="answer-key">${String.fromCharCode(65 + index)}</span><span>${option}</span></button>`).join('')}</div><div class="feedback" id="feedback" hidden></div></section><footer class="game-footer"><span class="mini-dot"></span><span>Una partida feta al barri, per al barri</span><button class="quit" type="button">Sortir</button></footer></main>`
...
function showPenaltyScreen(category: string, remaining: number): void {
  app.innerHTML = `<main class="shell penalty-screen"><div class="penalty-top"><div class="brand-mark" aria-hidden="true">PX</div><span class="penalty-score">Punts <strong>${score}</strong></span></div>${renderBoard()}<div class="comic-scene" aria-live="assertive"><div class="comic-caption">UPS!</div><div class="comic-character"><div class="character-hat"></div><div class="character-face"><i></i><i></i><b></b></div><div class="character-body"><span>?</span></div><div class="character-arm left"></div><div class="character-arm right"></div><div class="character-leg left"></div><div class="character-leg right"></div></div><div class="comic-bubble">On era aquesta...?</div><div class="comic-shadow"></div></div><p class="eyebrow">${category.toUpperCase()}</p><h1>No és aquesta.</h1><div class="penalty-readout"><strong>−1 punt</strong><span>Opció descartada. Sense pistes.</span></div><p class="penalty-count">Tornem-hi en <b>4</b></p></main>`
  let secondsLeft = 4
  penaltyTimer = window.setInterval(() => {
    secondsLeft -= 1
...
function startScreen(): void {
  ...
  app.innerHTML = `<main class="shell start-screen"><div class="start-top"><div class="brand-mark large" aria-hidden="true">PX</div><span class="live-pill"><span></span> Joc en proves</span></div><p class="eyebrow">TRIVIAL DEL BARRI</p><h1>Quant coneixes<br><em>el Pla Xic?</em></h1><p class="intro">Una partida curta per descobrir històries, llocs i veus que fan únic el nostre barri.</p><button class="primary" id="start" type="button">Començar la partida <span>↗</span></button><div class="start-note"><span>✦</span> ${questions.length} preguntes preparades<br><span>⌁</span> Dissenyat per jugar en colla</div></main>`
